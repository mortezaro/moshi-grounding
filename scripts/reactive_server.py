...
class ServerState:
    ...
    def _react_buffer(self, chunk_np):
        ...
        while tot > maxs and len(self._ubuf) > 1: tot -= len(self._ubuf.pop(0))
        if self._ucount % self._react_every == 0:
            try: self._update_emotion()
            except Exception as e: log("warning", f"reactive update failed: {e}")

    def _update_emotion(self):
        import numpy as _np, sphn as _sphn, os as _os, ser_dim as _sd
        pcm = _np.concatenate(self._ubuf)
        if len(pcm) < int(self.mimi.sample_rate) or _np.sqrt((pcm**2).mean()+1e-9) < 0.006: return
        w16 = _np.asarray(_sphn.resample(pcm, self.mimi.sample_rate, 16000), dtype=_np.float32)
        vad = _np.array(_sd.predict(self._proc, self._sermodel, self.device, [w16]))[0]
        self._val_ema = 0.6*self._val_ema + 0.4*float(vad[2])
        v = self._val_ema
        emo = "hap" if v > 0.55 else ("sad" if v < 0.40 else "neu")
        if emo != self._cur_emo:
            self._cur_emo = emo; _os.environ["COND_EMOTION"] = emo
            ct = get_condition_tensors(self.model_type, self.lm_gen.lm_model, 1, self.cfg_coef)
            self.lm_gen.condition_tensors = ct
            ns = self.lm_gen.lm_model.fuser.get_sum(ct)
            if ns is not None: ns = ns.to(self.lm_gen.lm_model.dtype)
