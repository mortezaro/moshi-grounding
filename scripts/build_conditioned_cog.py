#!/usr/bin/env python
"""Conditioned data with FULL cognitive stack: text_conditions={emotion, arousal, dominance}
per clip (from audeering VAD). Plain words (fluent), grounding as CONDITIONING inputs."""
...
def main():
    ...
    ap.add_argument("--out", default="/iopsstor/scratch/cscs/mrohania/datasets/DailyTalkCondCog")
    ...
    with open(os.path.join(a.out,f"shard_{a.shard}.jsonl"),"w") as jo:
        for wf in mine:
            ...
            json.dump({"alignments":al, "text_conditions":{"emotion":dom_e,"arousal":aro,"dominance":dm}}, open(os.path.join(ow,bn[:-4]+".json"),"w"))
