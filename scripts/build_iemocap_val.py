"""IEMOCAP with HUMAN valence labels -> text_conditions {emotion, valence}.
Label-quality test: does conditioning on human valence steer wider than SER valence?
Same dyadic-stereo reconstruction as build_iemocap_dyadic; valence thresholds from data percentiles."""
...
def main():
    ...
    for f in sorted(glob.glob(p + "/**/*.parquet", recursive=True)):
        ...
        t = pq.read_table(f, columns=["titre","start_time","end_time","valence","emotion","to_translate","audio"])
        rows += t.to_pylist()
    _vv = np.array([r["valence"] for r in rows if r.get("valence") is not None])
    VAL_LO, VAL_HI = float(np.percentile(_vv, 33)), float(np.percentile(_vv, 67))
    print(f"[{a.shard}] human valence thresholds lo={VAL_LO:.2f} hi={VAL_HI:.2f} (n={len(_vv)})")
                ...
                if len(mains) >= 2 and len(others) >= 1:
                    ...
                    for s, r in win_utts:
                        ...
                        if ch == 0:
                            ...
                            if e: emos.append(e)
                            if r["valence"] is not None: vals.append(r["valence"])
                    if aligns and emos and vals:
                        ...
                        val = b3(float(np.mean(vals)), VAL_LO, VAL_HI)
                        bn = f"{a.shard}_{n}"
                        sphn.write_wav(os.path.join(ow, bn+".wav"), buf, SR)
                        json.dump({"alignments": aligns,
                                   "text_conditions": {"emotion": emo, "valence": val}},
