#!/usr/bin/env python
"""Label-quality run: condition on SER VALENCE directly (continuous -> lo/mid/hi buckets),
the axis emotion-class labels failed to steer. text_conditions={emotion, valence}."""
...
def main():
    ...
    ap.add_argument("--out", default="/iopsstor/scratch/cscs/mrohania/datasets/DailyTalkCondVal")
    ...
    with open(os.path.join(a.out,f"shard_{a.shard}.jsonl"),"w") as jo:
        for wf in mine:
            ...
            val=b3(float(vad[:,2].mean()), _T["val_lo"], _T["val_hi"])   # VALENCE bucket (index 2)
            st[val]+=1
            bn=os.path.basename(wf); lk=os.path.join(ow,bn)
            if not os.path.lexists(lk): os.symlink(wf,lk)
            json.dump({"alignments":al, "text_conditions":{"emotion":dom_e,"valence":val}}, open(os.path.join(ow,bn[:-4]+".json"),"w"))
            jo.write(json.dumps({"path":f"data_stereo/{bn}","duration":d.shape[1]/SR})+"\n"); n+=1
    print(f"[{a.shard}] DONE {n}. valence dist:",dict(st))
