...
class ConditionProvider(nn.Module):
    ...
    def text_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(getattr(v, "_fsdp_wrapped_module", v), _BaseTextConditioner)]

    @property
    def tensor_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(getattr(v, "_fsdp_wrapped_module", v), _BaseTensorConditioner)]
    ...
    def prepare(self, inputs: tp.Sequence[ConditionAttributes]) -> tp.Dict[str, tp.Any]:
        ...
        for attribute, batch in chain(text.items(), tensors.items()):
            ...
            conditioner = getattr(conditioner, "_fsdp_wrapped_module", conditioner)
