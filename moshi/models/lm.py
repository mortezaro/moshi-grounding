...
class LMModel(StreamingContainer):
    ...
    def forward(
        ...
        if condition_tensors is None:
            ...
            if self.condition_provider is not None and condition_tensors is not None:
                from ..conditioners import ConditionType as _CT
                _vals = list(condition_tensors.values())
                if _vals and not isinstance(_vals[0], _CT):
                    condition_tensors = self.condition_provider(condition_tensors)
