...
def get_lora_moshi(
    ...
    if lora_weights is not None:
        ...
        # materialize any remaining meta params (new conditioner params absent from checkpoint)
        for _mn, _mod in model.named_modules():
            _own = list(_mod.parameters(recurse=False))
            if _own and any(_pp.is_meta for _pp in _own):
                _mod.to_empty(device=device, recurse=False)
                for _pp in _mod.parameters(recurse=False):
                    if _pp.dim() >= 2:
                        torch.nn.init.normal_(_pp, std=0.02)
                    else:
                        torch.nn.init.zeros_(_pp)
