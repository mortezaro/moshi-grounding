...
source /iopsstor/scratch/cscs/mrohania/moshi_env.sh; export COND_EMOTION=1; export COND_LR_MULT=100
...
srun --ntasks=$SLURM_NNODES --ntasks-per-node=1 bash -c "
  ...
  uenv run --view=default pytorch/v2.9.1:v2 -- \"\$MOSHI_PY\" -m torch.distributed.run \
    ...
    --master-addr=$MASTER_ADDR --master-port=$MASTER_PORT -m train example/scondup100dbg.yaml"
