...
C=/iopsstor/scratch/cscs/mrohania/moshi_runs/E_u12/checkpoints/checkpoint_000800/consolidated
echo "NODE=$(hostname) MODEL=E_u12 PORT=8903"
srun --ntasks=1 uenv run --view=default pytorch/v2.9.1:v2 -- "$MOSHI_PY" -m moshi.server --lora-weight $C/lora.safetensors --config-path $C/config.json --host 0.0.0.0 --port 8903 2>&1
